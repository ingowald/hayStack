...
//      std::cout << "last line was " << line << std::endl;
    ...
    void   CylindersFromFile::executeLoad(OnePartition &dataGroup) 
    {
    ...
    VMDCyls::VMDCyls(const ResourceSpecifier &data,
                     int thisPartID)
      : data(data),
    ...
    void VMDCyls::create(DataLoader *loader,
                         const ResourceSpecifier &dataURL)
    {
      for (int i=0;i<dataURL.numParts;i++)
        loader->addContent(new VMDCyls(dataURL,i));
    }
    
    size_t VMDCyls::projectedSize()
    { return (100/12) * divRoundUp((size_t)fileSize, (size_t)data.numParts); }
    
    void   VMDCyls::executeLoad(OnePartition &dataGroup)
    {
    ...
    std::string VMDCyls::toString() 
    {
      return "VMDCyls{fileName="+data.where+", part "+std::to_string(thisPartID)+" of "
